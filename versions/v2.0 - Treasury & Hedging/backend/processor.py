...
def get_provision_mode(inventory_oz: float, metal: str, entity: str = 'SABIS') -> dict:
    """
    Returns current provision mode and rate for a given inventory level.

    SABI / SABGB:
        Never use provision — rate is always 0%.

    SABIS only:
        Inventory < 0  → PROVISION active  (company is short — float position)
                         rate = gold 4.5% / silver 8%
        Inventory >= 0 → NO PROVISION      (stock on hand)
                         rate = 0%  (NOT the hurdle rate — no cost is applied)
    """
    # SABI and SABGB: never use provision
    if entity.upper() in ('SABI', 'SABGB'):
        return {
            'active':    False,
            'mode':      'NO PROVISION',
            'rate_pct':  0.0,
    ...
    # SABIS: provision only when short
...
def live_impact_preview(
    ...
    # Provision mode before and after (entity not passed to preview — defaults to SABIS behaviour)
